...
export function createAgentResult({ status, output = null, observations = [], validation = null, error = null }) { if (!['COMPLETED','WAITING_FOR_APPROVAL','FAILED'].includes(status)) throw new ValidationError(`Invalid AgentResult status: ${status}`); return Object.freeze({ status, output, observations, validation, error }); }
