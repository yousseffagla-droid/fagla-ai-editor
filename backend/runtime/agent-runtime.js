...
export class AgentRuntime {
  constructor({ toolRegistry, toolExecutor, permissionPolicy, approvalStore, auditLogger, taskStore = null }) {
    this.toolRegistry = toolRegistry; this.toolExecutor = toolExecutor; this.permissionPolicy = permissionPolicy;
    this.approvalStore = approvalStore; this.auditLogger = auditLogger; this.taskStore = taskStore;
  }
  ...
  async run({ request, task, project, workspace, agent, execution, memory = {} }) {
    ...
    const coding = agent.name === 'coding-agent-v1';
    await this.auditLogger.append({
      event: coding ? 'coding.task.created' : 'task.started',
      taskId: execution.taskId, projectId: execution.projectId, agentId: execution.agentId,
      workspaceId: execution.workspaceId, executionId: execution.executionId
    });
    try {
      ...
      const plan = await agent.plan({ ...context, task: currentTask });
      if (!plan || !Array.isArray(plan.steps)) throw new ValidationError('Agent plan must contain a steps array');

      for (const step of plan.steps) {
        if (!this.toolRegistry.has(step.tool)) throw new InvalidToolInvocationError('Unknown tool: ' + step.tool);
        const tool = this.toolRegistry.get(step.tool);
        await this.auditLogger.append({
          event: coding ? 'coding.tool.requested' : 'tool.requested',
          taskId: currentTask.id, agentId: execution.agentId, tool: tool.name, executionId: execution.executionId
        });
        ...
        if (decision.status === PERMISSION_DECISIONS.REQUIRES_APPROVAL) {
          ...
          await this.auditLogger.append({
            event: coding ? 'coding.approval.requested' : 'approval.requested',
            taskId: currentTask.id, approvalId: approval.id, tool: tool.name, executionId: execution.executionId
          });
      ...
      await this.auditLogger.append({ event: coding ? 'coding.validation.completed' : 'validation.completed', taskId: currentTask.id, executionId: execution.executionId });
      ...
      if (result.status === 'COMPLETED') {
        ...
        await this.auditLogger.append({ event: coding ? 'coding.completed' : 'task.completed', taskId: currentTask.id, executionId: execution.executionId });
      } else if (result.status === 'FAILED') {
        currentTask = transitionTask(currentTask, TASK_STATUSES.FAILED); await this.persist(currentTask);
        await this.auditLogger.append({ event: coding ? 'coding.failed' : 'task.failed', taskId: currentTask.id, executionId: execution.executionId });
      }
      return result;
    } catch (error) {
      if (currentTask.status !== TASK_STATUSES.FAILED && currentTask.status !== TASK_STATUSES.CANCELLED) {
        try { currentTask = transitionTask(currentTask, TASK_STATUSES.FAILED); await this.persist(currentTask); } catch {}
      }
