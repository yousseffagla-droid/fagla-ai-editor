...
export async function runAllowedCommand({ command, args = [], workspace, timeoutMs = 120000 }) {
  ...
  return new Promise((resolve, reject) => {
    ...
    let stdout = ''; let stderr = ''; const startedAt = Date.now();
    ...
    child.on('close', code => { clearTimeout(timer); resolve(Object.freeze({ command, args: [...args], code, stdout, stderr, durationMs: Date.now() - startedAt, success: code === 0 })); });
  });
}
