...
export class PermissionPolicy {
  evaluate(tool, executionContext) {
    ...
    if (tool.requiredPermission && !executionContext.permissions.includes(tool.requiredPermission)) return { status: PERMISSION_DECISIONS.DENY, reason: `Missing permission: ${tool.requiredPermission}` };
  ...
  assertAllowed(decision) { if (decision.status === PERMISSION_DECISIONS.DENY) throw new PermissionDeniedError(decision.reason); return decision; }
}
