...
function setup({results=[{title:'Source A',url:'https://example.com/a',snippet:'Market finding A'},{title:'Source A duplicate',url:'https://example.com/a',snippet:'duplicate'}],permissions=['RESEARCH']}={}){
 const audit=new AuditLogger(),registry=new ToolRegistry(),approvals=new InMemoryApprovalStore({auditLogger:audit});
 registerResearchTools({registry,searchProvider:{search:async()=>({results})}});
...
test('malformed search result fails safely',async()=>{const x=setup();const bad=new ToolRegistry();registerResearchTools({registry:bad,searchProvider:{search:async()=>({bad:true})}});const agent=new ResearchAgent({toolExecutor:new ToolExecutor(bad),auditLogger:x.audit,planner:new DeterministicResearchPlanner({queries:['x']})});const task=createResearchTask({question:'x'});await assert.rejects(()=>x.runtime.run({request:'x',task:{...task,status:'CREATED'},project:{id:'p'},workspace:x.root,agent,execution:x.execution}),ValidationError);});
